...
taryears = [[1961,2019],[1965,2023],[1970,2028]] #list containing the start and end years for forecast year: 1, 5 and 10 are considered so far
hemis = 'nh'
...
for sea in np.arange(len(seasons)):
    ...
    for lt in np.arange(len(lead_time)):
        ...
        for en in np.arange(len(ensemble)):
            ...
            start_year_step = taryears[lt][0]
            end_year_step = taryears[lt][1]
            ...
            print('INFO: get annual and decadal mean time series as well as signal-to-noise ratios for '+aggreg+' LWT counts, '+ensemble[en]+' with '+str(len(mrun[en]))+' runs, '+str(seasons[sea])+' months, '+str(taryears[lt])+' years, '+str(tarhours)+' hours. Output LWT frequency units are: '+yearly_units+'.')
            ...
            for mm in list(range(len(model))):
                ...
                if sea == 0 and lt == 0 and en == 0 and mm == 0: 
                    wt_agg = np.zeros((len(model),len(list(range(start_year_step,end_year_step+1))),len(wt.lon),len(wt.lat)))
                ...
                bin_array = np.zeros(wt.wtseries.shape)
                tarwt_ind = np.where(wt.wtseries.isin(tarwts))
                bin_array[tarwt_ind] = 1
                arr_tarwts = xr.DataArray(data=bin_array,coords=[pd.DatetimeIndex(wt.time),wt.lon,wt.lat],dims=['time','lon','lat'],name='wtseries')
            ...
            map_dir = figs+'/'+model[mm]+'/'+experiment[en]+'/'+hemis+'/'+aggreg+'/maps' #experiment-specific output figure directory
            #create target directory if missing
            if os.path.isdir(map_dir) != True:
                os.makedirs(map_dir)
            if os.path.isdir(map_dir) != True:
                os.makedirs(map_dir)
            ...
            # #runnoise.plot()
            # if experiment[en] == 'dcppA':
                # savename_stn = timeseries_dir+'/STN_'+seasons[sea]+'_'+model[mm]+'_'+experiment[en]+'_'+str(len(mrun))+'mem_'+wtlabel+'_'+str(lead_time[lt])+'y_ctr_'+center_wrt+'_'+str(start_year_step)+'_'+str(end_year_step)+'_'+seaslabel+'_'+aggreg+'_'+city[cc]+'_lon'+str(wt.lon.values)+'_lat'+str(wt.lat.values)+'.'+outformat
            # else:
                # savename_stn = timeseries_dir+'/STN_'+seasons[sea]+'_'+model[mm]+'_'+experiment[en]+'_'+str(len(mrun))+'mem_'+wtlabel+'_ctr_'+center_wrt+'_'+str(start_year_step)+'_'+str(end_year_step)+'_'+seaslabel+'_'+aggreg+'_'+city[cc]+'_lon'+str(wt.lon.values)+'_lat'+str(wt.lat.values)+'.'+outformat
            ...
            # if experiment[en] == 'dcppA':
                # savename_ts = timeseries_dir+'/timeseries_'+seasons[sea]+'_'+model[mm]+'_'+experiment[en]+'_'+str(len(mrun))+'mem_'+wtlabel.replace(" ","_")+'_'+str(lead_time[lt])+'y_ctr_'+center_wrt+'_'+str(start_year_step)+'_'+str(end_year_step)+'_'+city[cc]+'_lon'+str(wt.lon.values)+'lat'+str(wt.lat.values)+'.'+outformat
            # else:
                # savename_ts = timeseries_dir+'/timeseries_'+seasons[sea]+'_'+model[mm]+'_'+experiment[en]+'_'+str(len(mrun))+'mem_'+wtlabel.replace(" ","_")+'_ctr_'+center_wrt+'_'+str(start_year_step)+'_'+str(end_year_step)+'_'+city[cc]+'_lon'+str(wt.lon.values)+'lat'+str(wt.lat.values)+'.'+outformat
