...
test_level = 0.05 #test level in %, following the nomenclature used in Wilks 2006; used to obtaine the critical values for the RPC
...
lwts2plot = 8 #'from_data' or integer between 1 and 27. Specifies the upper tail of the discete colobar for plotting the number of Lamb Weather types that can be hindcast with significant correlation skill.

##execute ###############################################################################################
os.chdir(rundir)

if len(tarwts) != len(tarwts_name):
    raise ValueError('ERROR: the lenght of <tarwts> must equal the lenght of <tarwts_name> !')
        ...
        for en in np.arange(len(experiment)):
            ...
            for mm in list(range(len(model))):
                ...
                if experiment[en] in ('amip','dcppA','historical','piControl'):
                    ...
                    raise ValueError('ERROR: unknown entry in <experiment> input parameter')
                ...
                if experiment[en] == 'dcppA' and str(lead_time_concept[en]) == 'LT':
                    ...
                    raise ValueError('ERROR: check entry for experiment[en] !')
                ...
                if seasons[sea] in ('MAM','JJA','SON'):
                    ...
                    raise ValueError('ERROR: check entry for <seasons[sea]> !')
                ...
                if detrending == 'yes':
                    ...
                    raise ValueError('ERROR: check entry for <detrending> input parameter !')
...
if anom == 'no':
    ...
    raise ValueError('ERROR: check entry for <anom> input parameter !')
...
if correlation_type == "Pearson":
    ...
    raise ValueError('ERROR: check entry for <correlation_type> input parameter !')
...
if rpc_method in ('Eade','Cottrell'):
    ...
    raise ValueError('ERROR: check entry for <rpc_method> input parameter !')
rpc_sigind = p_value_rpc < test_level
rpc_sigind = xr.DataArray(rpc_sigind,coords=rpc.coords,dims=rpc.dims,name='sig_of_diff_modelled_vs_observed_pc')

#and plot the results
#init numpy arrays to be filled in the loop below
p_value_rho_diff = np.zeros((len(tarwts),len(seasons),len(experiment),len(rho.lon),len(rho.lat)))
p_value_rho_diff[:] = np.nan
...
for lwt in np.arange(len(tarwts_name)):
    for sea in np.arange(len(seasons)):
        for en in np.arange(len(experiment_out)):
            ...
            sig_ind_nh = (rho_pval_nh.values < test_level) & (rho_nh.values > 0)
            sig_ind_sh = (rho_pval_sh.values < test_level) & (rho_sh.values > 0)
            ...
            sig_ind_nh = p_value_rho_diff_nh.values < test_level
            sig_ind_sh = p_value_rho_diff_sh.values < test_level
            ...
            # assign
            p_value_rho_diff[lwt,sea,en,:,:] = p_value_rho_diff_step.values
...
p_value_rho_diff = xr.DataArray(p_value_rho_diff, coords=[tarwts_name,seasons, experiment_out, lon, lat ], dims=['lwt','season','experiment','lon','lat'], name='pvalue')
p_value_rho_diff.attrs['units'] = 'p-value for the difference in the Pearson correlation coefficient (probability)'
...
pc_mod_nh = pc_mod.isel(lat=rho.lat >= 0)
pc_mod_sh = pc_mod.isel(lat=rho.lat < 0)
...
critical_value = t.ppf(1 - test_level / 2, df) #calculate the critical value
...
for sea in np.arange(len(seasons)):
    ...
    for en in np.arange(len(experiment_out)):
        ...
        if rpc_method == 'Scaife':
            ...
            raise ValueError('Check entry for <rpc_method> input parameter !')
...
#plot the sum of LWTs for which a significant hindcast is obtained at each grid box, do this for the entire globe and separately for NH and SH
sig_skill_sum = ((rho_pval < test_level) & (rho > 0)).sum(dim='lwt')
sig_addedskill_sum = ((p_value_rho_diff < test_level) & (p_value_rho_diff > 0)).sum(dim='lwt')

for sea in np.arange(len(seasons)):
    for en in np.arange(len(experiment_out)):
        savedir_sum = figs+'/'+experiment[en]+'/global/summary/correlation/maps'
        if os.path.isdir(savedir_sum) != True:
            os.makedirs(savedir_sum)
        # sig_skill_sum_step = sig_skill_sum.sel(season=seasons[sea],experiment=experiment_out[en])
        cbarlabel_skill_sum = 'Nr. of predictable LWTs (out of 11)'
        cbarlabel_addedskill_sum = 'Nr. of LWTs with added skill (out of 11)'
        title_skill_sum = 'Nr. of predictable LWTs for '+experiment_out[en]+' dtr'+detrending+', '+seasons[sea]+', '+str(taryears[0])+', '+str(taryears[1])
        title_addedskill_sum = 'Nr. of LWTs with added skill for '+experiment_out[en]+' dtr'+detrending+', '+seasons[sea]+', '+str(taryears[0])+', '+str(taryears[1])
        savename_nh_skill_sum = savedir_sum+'/predictable_LWTs_'+anom+'_dtr_'+detrending+'_'+seasons[sea]+'_'+experiment_out[en]+'_nh_'+str(taryears[0])+'_'+str(taryears[1])+'.'+outformat
        savename_sh_skill_sum = savedir_sum+'/predictable_LWTs_'+anom+'_dtr_'+detrending+'_'+seasons[sea]+'_'+experiment_out[en]+'_sh_'+str(taryears[0])+'_'+str(taryears[1])+'.'+outformat
        savename_nh_addedskill_sum = savedir_sum+'/added_skill_LWTs_'+anom+'_dtr_'+detrending+'_'+seasons[sea]+'_'+experiment_out[en]+'_nh_'+str(taryears[0])+'_'+str(taryears[1])+'.'+outformat
        savename_sh_addedskill_sum = savedir_sum+'/added_skill_LWTs_'+anom+'_dtr_'+detrending+'_'+seasons[sea]+'_'+experiment_out[en]+'_sh_'+str(taryears[0])+'_'+str(taryears[1])+'.'+outformat
        if lwts2plot == 'from_data':
            max2plot = max([sig_addedskill_sum.max(),sig_skill_sum.max()]).values
            map_polar_single_field(sig_skill_sum.sel(season=seasons[sea],experiment=experiment_out[en]),title_skill_sum,savename_nh_skill_sum,savename_sh_skill_sum,0,max2plot,dpival,colormap,titlesize,cbarlabel_skill_sum,cbar_norm_f=max2plot)
            map_polar_single_field(sig_addedskill_sum.sel(season=seasons[sea],experiment=experiment_out[en]),title_addedskill_sum,savename_nh_addedskill_sum,savename_sh_addedskill_sum,0,max2plot,dpival,colormap,titlesize,cbarlabel_addedskill_sum,cbar_norm_f=max2plot)
        elif lwts2plot > 0 & lwts2plot <= 27: 
            map_polar_single_field(sig_skill_sum.sel(season=seasons[sea],experiment=experiment_out[en]),title_skill_sum,savename_nh_skill_sum,savename_sh_skill_sum,0,lwts2plot,dpival,colormap,titlesize,cbarlabel_skill_sum,cbar_norm_f=lwts2plot)
            map_polar_single_field(sig_addedskill_sum.sel(season=seasons[sea],experiment=experiment_out[en]),title_addedskill_sum,savename_nh_addedskill_sum,savename_sh_addedskill_sum,0,lwts2plot,dpival,colormap,titlesize,cbarlabel_addedskill_sum,cbar_norm_f=lwts2plot)
        else:
            raise ValueError('Unknown entry for <lwts2plot> input parameter!')
        # sig_skill_sum_step.close()
sig_skill_sum.close()
sig_addedskill_sum.close()

#calculate pattern correlation between skill and ratio of predictable components, do this for the entire domain (NH+SH) and separately for NH and SH
...
#calculate pattern correlation between skill and predictable component in the model, do this for the entire domain (NH+SH) and separately for NH and SH
pattern_r_rho_pc_mod = xr.corr(rho.stack(lon_lat=('lon','lat')),pc_mod.stack(lon_lat=('lon','lat')),dim='lon_lat')
pattern_r_rho_pc_mod_nh = xr.corr(rho_nh.stack(lon_lat=('lon','lat')),pc_mod_nh.stack(lon_lat=('lon','lat')),dim='lon_lat')
pattern_r_rho_pc_mod_sh = xr.corr(rho_sh.stack(lon_lat=('lon','lat')),pc_mod_sh.stack(lon_lat=('lon','lat')),dim='lon_lat')

#calculate pattern correlation between skill and added value, do this separately for NH and SH, and for FY1 and 2
#for forecast year 1
rho_diff_fy1_nh = rho_nh.sel(experiment='dcppA_fy1') - rho_nh.sel(experiment='historical')
rho_diff_fy1_sh = rho_sh.sel(experiment='dcppA_fy1') - rho_sh.sel(experiment='historical')
pattern_r_rho_diff_fy1_nh = xr.corr(rho_nh.sel(experiment='dcppA_fy1').stack(lon_lat=('lon','lat')),rho_diff_fy1_nh.stack(lon_lat=('lon','lat')),dim='lon_lat')
pattern_r_rho_diff_fy1_sh = xr.corr(rho_sh.sel(experiment='dcppA_fy1').stack(lon_lat=('lon','lat')),rho_diff_fy1_sh.stack(lon_lat=('lon','lat')),dim='lon_lat')
#for forecast year 2
rho_diff_fy2_nh = rho_nh.sel(experiment='dcppA_fy2') - rho_nh.sel(experiment='historical')
rho_diff_fy2_sh = rho_sh.sel(experiment='dcppA_fy2') - rho_sh.sel(experiment='historical')
pattern_r_rho_diff_fy2_nh = xr.corr(rho_nh.sel(experiment='dcppA_fy2').stack(lon_lat=('lon','lat')),rho_diff_fy2_nh.stack(lon_lat=('lon','lat')),dim='lon_lat')
pattern_r_rho_diff_fy2_sh = xr.corr(rho_sh.sel(experiment='dcppA_fy2').stack(lon_lat=('lon','lat')),rho_diff_fy2_sh.stack(lon_lat=('lon','lat')),dim='lon_lat')
...
rho_pval.close()
rho_pval_effn.close()
rho_diff_fy1_nh.close()
rho_diff_fy2_nh.close()
rho_diff_fy1_sh.close()
rho_diff_fy2_sh.close()
pearson_r.close()
pearson_pval.close()
...
pattern_r_rho_diff_fy1_nh.close()
pattern_r_rho_diff_fy2_nh.close()
pattern_r_rho_diff_fy1_sh.close()
pattern_r_rho_diff_fy2_sh.close()
