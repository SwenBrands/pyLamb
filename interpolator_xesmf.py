...
if experiment in ('amip','historical'):
    startdate = "01/01/1979 00:00:00" # date format is month/day/year, currently not used
    enddate = "31/12/2005 18:00:00" #currently not used
    #taryears=[1979,2005]
    taryears=[1850,2014]
elif experiment in ('ssp126','ssp245','ssp370','ssp585'):
     startdate = "01/01/2015 00:00:00" # date format is month/day/year, currently not used
     enddate = "31/12/2100 18:00:00" #currently not used
     taryears=[2015,2100]
elif experiment == '20c':
     startdate = "01/01/2015 00:00:00" # date format is month/day/year, currently not used
     enddate = "31/12/2100 18:00:00" #currently not used
...
for j in list(range(len(model))):
    if model[j] in ('interim','era5'):
        latname = 'latitude' #'latitude'
        lonname = 'longitude' #'longitude'
        varname = 'msl'
        timestep = '6h'
    elif model[j] == 'ec_earth3' and mrun[j] == 'r2i1p1f1':
        latname = 'latitude'
        lonname = 'longitude'
        varname = 'psl'
        timestep = '6h'
    elif model[j] == 'cera20c':
        latname = 'latitude' #'latitude'
        lonname = 'longitude' #'longitude'
        varname = 'msl'
        timestep = '3h'
    ...
    prev_tmpdir = root1 + model[j] + root2 + mrun[j]+'/psl_interp'
    ...
    listdir_raw = os.listdir(root1 + model[j] + root2 + mrun[j])
    dropind = []
    ...
    for infile in listdir:
        fullpath = root1 + model[j] + root2 + mrun[j]+'/'+infile
        ...
        if model[j] in ('interim','era5','cera20c'):
        ...
        if model[j] == 'canesm5':
            print('INFO: only hours 1:30, 7:30, 13:30 and 19:30 are retained from '+model[j]+ ' dataset...')
        ...
        print('INFO: The calendar for '+model[j]+' is '+str(calendar_from_nc)+'.')
        ...
        if model[j] in ('interim','era5','jra55','cera20c'):
        ...
        if (yearinds.size == 1) & (dates.year[-1] == 1979) & (model[j] in ('miroc6','fgoals_g2','cnrm_cm5','miroc_es2l','awi_esm_1_1_lr','hadgem3_gc31_mm')):
            print('INFO: Exception is inlcuded for '+model[j]+', and year '+str(dates.year[-1])+' with a single timestamp on 1979-01-01 OO:00:00: The index number for the prior date is included to have two indices for loading the nc file with xr.load_dataset(). This dummy index deleted afterwards.')
        ...
        print(dataset)
        if model[j] in ('cnrm_cm6_1','cnrm_cm6_1_hr'): 
            print('Dropping variable <time_bounds> for '+model[j]+' and '+mrun[j]+'...')
            dataset = dataset.drop(labels='time_bounds')
        elif model[j] == 'canesm5': #exception for 3-hourly psl data from CanESM5
            print('Dropping variable <time_bnds>, <lat_bnds> and <lon_bnds> for '+model[j]+' and '+mrun[j]+'...')
        ...
        tmpfile = root1 + model[j] + root2 + mrun[j] + '/psl_interp' + '/tmp_interp_'+timestep+'rPlev_' + model[j] +'_'+experiment+'_' + mrun[j] + '_' + hemis +'_'+starthour+'_'+endhour+'.nc'
    ...
    tmpfiles = root1 + model[j] + root2 + mrun[j] + '/psl_interp' + '/tmp_interp_'+timestep+'rPlev_' + model[j] +'_'+experiment+'_' + mrun[j] + '_' + hemis +'*.nc' 
    ...
    #newfile = root1 + model[j] + root2 + mrun[j] + '/psl_interp' + '/psl_interp_6hrPlev_' + model[j] +'_'+experiment+'_' + mrun[j] + '_' + hemis +'_'+starthour_all+'_'+endhour_all+'.nc'
    newfile = root1 + model[j] + root2 + mrun[j] + '/psl_interp' + '/psl_interp_'+timestep+'rPlev_' + model[j] +'_'+experiment+'_' + mrun[j] + '_' + hemis +'.nc'
    ...
    print('INFO: Regridding for '+model[j]+', '+experiment+', '+mrun[j]+', '+hemis+', '+str(taryears).replace('[','').replace(']','').replace(', ',' to ')+' has been accomplished. See if any temporary files remain in '+tmpfiles)
    print(' ')
    print('------------ THE REGRIDDING PROCEDURE FOR '+model[j]+', '+experiment+', '+mrun[j]+', '+hemis+', '+str(taryears).replace('[','').replace(']','').replace(', ',' to ')+' WAS SUCCESSFUL -----------------------------------------')
    print(' ')
